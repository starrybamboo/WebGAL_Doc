...
`vocal` は音声タイムラインを選択し、`left`、`right`、`center`、`figureId` は統一された顔ランタイムが駆動するキャラクターを指定します。画像キャラクターでは、キャラクターディレクトリの `figure.json` に `facialRig` を定義し、`character` コマンドで表示してください。`changeFigure` で全身画像の口差分を登録することはできません。このワークフローは Live2D や Spine には適用されません。

完全なワークフローは[アニメーション - 画像キャラクターの顔アニメーション](../../webgal-script/animation.md#画像キャラクターの顔アニメーション)を参照してください。
